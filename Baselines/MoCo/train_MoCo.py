...
from moco_load_data import MoCoDataset
from Baselines.Models.MultiMoCoCreator import MoCo1D as MoCo
import numpy as np
import random
import argparse
def args_parse():
...
train_dataset = MoCoDataset('../../Dataset/data_'+dataset+'_segments/person_all_idx.txt', train_transform)
train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, drop_last=True)

moco = MoCo(device=device, base_model=args.baseModel, view='signal').to(device)
...
for epoch in range(epochs):
    ...
    if loss < best_loss:
        best_loss = loss
        torch.save(moco, './MoCoModel_{}_{}_signal.pth'.format(args.dataset, args.baseModel))
