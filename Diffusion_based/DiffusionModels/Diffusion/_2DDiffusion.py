import sys
sys.path.append('../diffusionModels')
from Diffusion_based.DiffusionModels.utils._2DnetworkHelper import *
from Diffusion_based.DiffusionModels.Diffusion._2DvarianceSchedule import VarianceSchedule
    ...
    def forward(self, mode, **kwargs):
        if mode == "train":
            if "x_start" and "t" in kwargs.keys():
                ...
                raise ValueError("The diffusion model must be trained by passing in the parameters x_start and t！")
            ...
            if "image_size" and "batch_size" and "channels" in kwargs.keys():
                ...
                raise ValueError("The diffusion model must pass three parameters, including image_size, batch_size, and channels, when generating images.")
        elif mode == "feature_extractor":
            return self.feature_extractor(x_start=kwargs["x_start"], t=kwargs["t"], noise=None)
        else:
            raise ValueError("The mode parameter must be selected from the two modes of {train} and {generate}.")
